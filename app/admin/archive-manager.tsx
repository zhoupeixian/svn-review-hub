  ...
  return <section className="mt-8 grid gap-8">
    <section className="rounded-3xl border border-[#d8e4d9] bg-white p-6 shadow-[0_12px_34px_rgba(31,77,51,0.05)] sm:p-8">
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-4">
        ...
        <label>问题状态<select aria-label="按问题状态筛选活动日志" value={filters.status ?? ''} onChange={(e) => setFilters({ ...filters, status: e.target.value })}><option value="">全部状态</option>{ISSUE_STATUSES.map((status) => <option key={status} value={status}>{ISSUE_STATUS_LABELS[status]}</option>)}</select></label>
        <label>关键词<input aria-label="按关键词筛选活动日志" value={filters.keyword ?? ''} onChange={(e) => setFilters({ ...filters, keyword: e.target.value })} /></label>
        <label>级别<select aria-label="按问题级别筛选活动日志" value={filters.severity ?? ''} onChange={(e) => setFilters({ ...filters, severity: e.target.value })}><option value="">全部级别</option><option value="P1">P1</option><option value="P2">P2</option><option value="P3">P3</option></select></label>
        <button type="button" onClick={applyFilters} disabled={busy} className="self-end rounded-lg border border-[#b9cbbb] bg-white px-4 py-2.5 text-sm font-bold text-[#1d5b46]">应用筛选</button>
      </div>
