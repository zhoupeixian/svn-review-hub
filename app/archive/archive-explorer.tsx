...
export default function ArchiveExplorer({
  ...
  return (
    <section className="mt-8">
      ...
      <div className="share-actions mb-5"><div><p className="text-sm font-bold text-[#243e31]">分享当前筛选</p><p className="mt-1 text-xs text-[#718077]">复制链接后可发送给团队成员，导出将沿用同一筛选范围。</p></div><div className="flex flex-wrap gap-2"><button type="button" onClick={copyShareLink} className="filter-quiet">{copied ? '已复制' : '复制当前筛选链接'}</button>{exportHref && <a href={exportHref} className="filter-apply">导出归档清单</a>}</div></div>
