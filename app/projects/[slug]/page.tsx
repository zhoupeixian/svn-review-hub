    ...
    <main className="min-h-screen bg-[#f6f7f3] text-[#17211d]">
      ...
      <section className="mx-auto max-w-7xl px-5 pb-10 sm:px-8">
        <div className="rounded-2xl border border-[#d9e4da] bg-white p-5">
          ...
          {health.parseFailure && <p role="alert" className="mt-4 text-sm font-semibold text-[#a44138]">最近一次自动接收未解析出有效提交，请检查日志格式。</p>}
          {health.zeroIssueWarning && !health.parseFailure && <p role="status" className="mt-4 text-sm font-semibold text-[#8a6a2b]">最近一次自动接收没有提取到问题，请确认审查结果是否完整。</p>}
        </div>
      </section>
      <ReviewExplorer
